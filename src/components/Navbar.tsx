          ...
          <div className="hidden md:flex items-center justify-center gap-8 z-20">
            {navLinks.map((link) => (
              <Link
                key={link.href}
                href={link.href}
                className={`text-sm font-semibold relative py-2 transition-all duration-300 text-gray-700 hover:text-[#0284c7] whitespace-nowrap ${pathname === link.href ? 'text-[#0284c7]' : ''
                  }`}
              >
                {link.label}
                {pathname === link.href && (
                  <motion.span
                    ...
                    className="absolute bottom-0 left-0 right-0 h-0.5 bg-[#0284c7] rounded-full"
            ...
            <div className="flex flex-col gap-4">
              {navLinks.map((link) => (
                <Link
                  ...
                  className={`text-base font-semibold py-2 border-b border-gray-50 text-gray-700 hover:text-[#0284c7] transition-colors ${pathname === link.href ? 'text-[#0284c7]' : ''
                    }`}
                >
                  {link.label}
                </Link>
