    ...
    <>
      ...
      <section className="relative pt-36 sm:pt-44 lg:pt-52 pb-14 sm:pb-16 overflow-hidden" style={{ background:'linear-gradient(135deg,#0f172a 0%,#1e293b 50%,#0f172a 100%)' }}>
        <div className="absolute inset-0 overflow-hidden pointer-events-none">
          <div className="absolute w-[600px] h-[600px] rounded-full opacity-15 blur-3xl" style={{ background:'radial-gradient(circle,#0284c7,transparent 70%)', top:'-15%', left:'-10%' }} />
          <div className="absolute w-[400px] h-[400px] rounded-full opacity-10 blur-3xl" style={{ background:'radial-gradient(circle,#38bdf8,transparent 70%)', bottom:'-10%', right:'-5%' }} />
        </div>
        <div className="container-custom relative z-10 w-full">
          <motion.div initial={{ opacity:0, y:30 }} animate={{ opacity:1, y:0 }} transition={{ duration:0.7 }} className="text-center">
            <div className="inline-flex items-center justify-center gap-2 bg-white/5 text-sky-400 border border-sky-500/20 px-5 py-2 rounded-full text-xs font-bold uppercase mb-6 backdrop-blur-sm">
              <Globe size={14} /> KÜRESEL MEZUN AĞI
            </div>
            <h1 className="font-display text-4xl sm:text-5xl lg:text-6xl font-bold mb-5 text-white">
              Mezun <span className="text-transparent bg-clip-text bg-gradient-to-r from-sky-400 to-cyan-300">Haritası</span>
            </h1>
          ...
          <motion.div initial={{ opacity:0, y:10 }} animate={{ opacity:1, y:0 }} transition={{ delay:0.2 }} className="mb-6">
            <div className="relative">
              ...
              <input type="text" placeholder="Ülke veya şehir ara..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)}
                className="w-full pl-11 pr-4 py-3 bg-white/5 border border-white/10 rounded-xl text-sm text-white placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-sky-500/30 focus:bg-white/[0.07] transition-all backdrop-blur-sm" />
            </div>
          </motion.div>
                ...
                return (
                  <div key={country} className="rounded-2xl border border-white/[0.06] overflow-hidden">
                    ...
                    <button onClick={() => setExpandedCountry(isExpanded ? null : country)}
                      className={`w-full flex items-center justify-between p-4 transition-all ${isExpanded ? 'bg-sky-500/10' : 'bg-white/[0.03] hover:bg-white/[0.06]'}`}>
                      ...
                      <div className="flex items-center gap-2">
                        <span className={`text-xs font-extrabold px-2.5 py-1 rounded-full ${isExpanded ? 'bg-sky-500 text-white' : 'bg-white/10 text-slate-300'}`}>{group.total}</span>
                            ...
                            {group.cities.map(city => {
                              ...
                              return (
                                <button key={city.id} onClick={() => setSelectedCity(isActive ? null : city)}
                                  className={`w-full text-left p-3 rounded-xl border transition-all ${isActive ? 'bg-sky-500/10 border-sky-500/30' : 'bg-white/[0.02] border-white/[0.04] hover:bg-white/[0.05]'}`}>
                                  <div className="flex items-center justify-between mb-1.5">
                                    <span className={`font-semibold text-xs ${isActive ? 'text-sky-400' : 'text-slate-300'}`}>{city.city}</span>
